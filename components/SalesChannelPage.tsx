          ...
          <Table>
            <TableHeader>
              <TableRow>
                ...
                <TableHead>取扱アイテム</TableHead>
                <TableHead className="w-24" />
              </TableRow>
            </TableHeader>
            <TableBody>
              {platforms.map((platform) => {
                const itemsForPlatform = items.filter((item) =>
                  (item.platformSettings ?? []).some((s: any) => s.platformId === platform.id),
                );
                ...
                return (
                  <TableRow key={platform.id}>
                    ...
                    <TableCell className="max-w-md">
                      {itemsForPlatform.length === 0 ? (
                        <span className="text-xs text-muted-foreground">未設定</span>
                      ) : (
                        <div className="space-y-1 text-xs">
                          {itemsForPlatform.map((item) => {
                            const variantTypes =
                              item.variants?.map((v) => v.type).join(" / ") ?? "バリエーションなし";
                            return (
                              <div key={item.id} className="flex flex-wrap gap-1">
                                <span className="font-medium">{item.name}</span>
                                <span className="text-muted-foreground">（{variantTypes}）</span>
                              </div>
                            );
                          })}
                        </div>
